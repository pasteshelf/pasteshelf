...
public final class MarkdownFormatterPlugin: NSObject, PasteShelfPlugin {
    ...
    private func convertHTMLToMarkdown(_ html: String) -> String {
        ...
        // Bold
        result = result.replacingOccurrences(of: "<(strong|b)>(.*?)</\\1>", with: "**$2**", options: .regularExpression)
        ...
        // Links
        result = result.replacingOccurrences(of: "<a[^>]*href=\"([^\"]*)\"[^>]*>(.*?)</a>", with: "[$2]($1)", options: .regularExpression)

        // Images
        result = result.replacingOccurrences(of: "<img[^>]*src=\"([^\"]*)\"[^>]*alt=\"([^\"]*)\"[^>]*/?>", with: "![$2]($1)", options: .regularExpression)
        result = result.replacingOccurrences(of: "<img[^>]*src=\"([^\"]*)\"[^>]*/?>", with: "![]($1)", options: .regularExpression)
        ...
        // Blockquotes
        result = result.replacingOccurrences(of: "<blockquote[^>]*>(.*?)</blockquote>", with: "> $1\n", options: .regularExpression)
