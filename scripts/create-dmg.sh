#!/bin/bash
# create-dmg.sh — Create a branded macOS DMG installer for PasteShelf.
#
# Usage: ./scripts/create-dmg.sh <version> [path/to/PasteShelf.app]
#
...
# Defaults
# ---------------------------------------------------------------------------
DEFAULT_APP_PATH="$PROJECT_ROOT/build/Build/Products/Release/PasteShelf.app"
OUTPUT_DIR="$PROJECT_ROOT/build"
BACKGROUND_IMAGE="$PROJECT_ROOT/assets/dmg-background.png"
VOLUME_NAME="PasteShelf"
...
usage() {
    cat <<EOF
Usage: $(basename "$0") <version> [path/to/PasteShelf.app]

Create a branded macOS DMG installer for PasteShelf.

Arguments:
  version                 Release version string (e.g. 1.0.0)
  path/to/PasteShelf.app  Path to the built .app bundle
...
Examples:
  ...
  ./scripts/create-dmg.sh 1.0.0 /path/to/Release/PasteShelf.app
EOF
}
...
if command -v create-dmg &>/dev/null; then
    ...
    CREATE_DMG_ARGS=(
        ...
        --window-size 600 400
        --icon-size 128
        --icon "PasteShelf.app" 150 190
        --app-drop-link 450 190
        --hide-extension "PasteShelf.app"
    )
...
TEMP_DMG="$OUTPUT_DIR/PasteShelf-temp.dmg"
...
tell application "Finder"
    tell disk "$VOLUME_NAME"
        ...
        set position of item "PasteShelf.app" of container window to {150, 190}
