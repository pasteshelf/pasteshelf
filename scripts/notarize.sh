...
#
# Usage:
#   Local:  ./scripts/notarize.sh path/to/PasteShelf.app
#   CI:     APPLE_ID=... APPLE_TEAM_ID=... APPLE_APP_PASSWORD=... CI=true ./scripts/notarize.sh path/to/PasteShelf.dmg
#
# Modes:
#   Keychain profile (local dev): Credentials are stored via:
#     xcrun notarytool store-credentials "PasteShelf-Notarize" \
...
KEYCHAIN_PROFILE="PasteShelf-Notarize"
TIMEOUT="30m"
...
Examples:
  ./scripts/notarize.sh build/PasteShelf.app
  ./scripts/notarize.sh build/PasteShelf.dmg
